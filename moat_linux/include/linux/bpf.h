...
struct moat_pack {
  ...
  pgd_t moat_pgd_val;
  pgd_t init_pgd_val;
  u64* __percpu percpu_stack;
  u64* __percpu percpu_bpf_stack;
  int pcid;
  
  pte_t ** __percpu ctx_ppte;
  u64* __percpu percpu_ctx;
};
...
struct bpf_prog {
	...
	int __percpu		*active;
  struct moat_pack pack;
	unsigned int		(*bpf_func)(const void *ctx,
					    const struct bpf_insn *insn);
  unsigned int    (*jitted_image)(const void *ctx,
              const struct bpf_insn *insn);
	struct bpf_prog_aux	*aux;		/* Auxiliary fields */
	struct sock_fprog_kern	*orig_prog;	/* Original BPF program */
  struct bpf_insn_aux_data *insn_aux; /* used by MOAT */
  struct moat_addrspace *addrspace; /* expected address space for the program */
  struct moat_mem *prog_mem; /* moat_mem for insn & image */
  struct moat_mem __percpu **stacks; /* percpu pointer to stacks */
  struct moat_mem __percpu **bpf_stacks; /* used by interpreter */
  unsigned int		(*moat_func)(const void *ctx,
					    const struct bpf_insn *insn);
  atomic_t __percpu *cpu_stack_cnt; /* stack usage for each cpu */
