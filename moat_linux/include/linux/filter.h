...
#define BPF_FIELD_SIZEOF(type, field)                                 \
	({                                                            \
		const int __size =                                    \
			bytes_to_bpf_size(sizeof_field(type, field)); \
		BUILD_BUG_ON(__size < 0);                             \
		__size;                                               \
	})
...
#define __BPF_CAST(t, a)                                                      \
	(__force t)(__force typeof(__builtin_choose_expr(                     \
		sizeof(t) == sizeof(unsigned long), (unsigned long)0, (t)0))) \
		a
#define __BPF_V void
#define __BPF_N
...
static u32 __attribute__((naked))
asm_moat_jit_run(const void *ctx, void *bpf_func, int dummy, int PKRS,
		 u64 stack)
{
	/* 
   * ctx: %rdi
   ...
   */
	/*
   * ENTER SEQ: mov $1, in_bpf -> wrmsr
   * EXIT SEQ: wrmsr -> mov $0, in_bpf
   */
	asm("mov %%rsp, %%rax\n\t"
	    "mov %%r8, %%rsp\n\t"
	    "push %%rax\n\t"
...
//static u64 t_array[10000];
extern u64 moat_sum;
extern atomic_t moat_count;
extern spinlock_t moat_lock;
extern u64 get_asid(void);
					  ...
					  bpf_dispatcher_fn dfunc)
{
	...
	if (static_branch_unlikely(&bpf_stats_enabled_key)) {
		...
		if (likely(prog->addrspace)) {
//#define MOAT_OFFSET 0xfc0
			//pgd_t *moat_pgd;
			u64 stack, borrow = 1;
			//void **last_ptr;
			//unsigned long flags;
			...
			//list_moat_regions_local(prog->addrspace);
			//list_moat_regions(prog->addrspace);
			//rdtscp_begin(high0, low0);
			stack = *arch_raw_cpu_ptr(prog->pack.percpu_stack);
			//moat_pgd = current->active_mm->pgd + (MOAT_OFFSET / 8);
			//moat_pgd->pgd = prog->pack.moat_pgd_val.pgd;
			//cr3 = __read_cr3();
			// we try hijack current first, then do other things
			borrow = atomic_cmpxchg(&current->active_mm->bpf_using, 0, 1);
			if (!borrow) {
				(current->active_mm->pgd + pgd_index(MOAT_START))->pgd = prog->pack.moat_pgd_val.pgd;
			} else 
				write_cr3(__pa(prog->pack.addrspace_pgd) | prog->pack.pcid | CR3_NOFLUSH);
			if (prog->type == BPF_PROG_TYPE_XDP) {
        pks_assign_in_pgd(init_mm.pgd, (u64)((struct xdp_buff *)ctx)->data, 0x1);
				asm volatile("invlpg (%0)" ::"r"(((struct xdp_buff *)ctx)->data) : "memory");
			} else {
        pks_assign_in_pgd(init_mm.pgd, (u64)ctx, 0x1);
				asm volatile("invlpg (%0)" ::"r"(ctx) : "memory");
      }
			ret = asm_moat_jit_run(ctx, prog->bpf_func, 0, 0x6E1, stack);
			// we delayed the switch to "switch_to"
			// if we are in a bloody user thread, no delay.
			if (!borrow) {
				(current->active_mm->pgd + pgd_index(MOAT_START))->pgd = prog->pack.init_pgd_val.pgd;
				atomic_set(&current->active_mm->bpf_using, 0);
			} else
				write_cr3(__pa(current->active_mm->pgd) | get_asid() | CR3_NOFLUSH);
      return ret;
			//printk("EXPECTED PCID %ld, REAL PCID %llu\n", __native_read_cr3() & 0xFFF, get_asid());
			//moat_pgd->pgd = prog->pack.init_pgd_val.pgd;
			//rdtscp_end(high1, low1);

			//if (atomic_inc_return(&moat_count) < 100000)
			//  moat_sum += (high1 << 32) + low1 - (high0 << 32) - low0;
			//else
			//printk("[MOAT] average ns: %llu", moat_sum / 100000);
 ...
 */
static inline void bpf_compute_and_save_data_end(struct sk_buff *skb,
						 void **saved_data_end)
{
	struct bpf_skb_data_end *cb = (struct bpf_skb_data_end *)skb->cb;
...
static inline void bpf_restore_data_end(struct sk_buff *skb,
					void *saved_data_end)
{
	struct bpf_skb_data_end *cb = (struct bpf_skb_data_end *)skb->cb;
...
{
	return round_up(bpf_prog_insn_size(prog) + sizeof(__be64) + 1,
			SHA1_BLOCK_SIZE);
}
...
static inline bool bpf_ctx_narrow_access_ok(u32 off, u32 size, u32 size_default)
{
	return size <= size_default && (size & (size - 1)) == 0;
}

static inline u8 bpf_ctx_narrow_access_offset(u32 off, u32 size,
					      u32 size_default)
{
...
#define bpf_ctx_wide_access_ok(off, size, type, field)            \
	(size == sizeof(__u64) && off >= offsetof(type, field) && \
...
int xdp_do_redirect_frame(struct net_device *dev, struct xdp_buff *xdp,
			  struct xdp_frame *xdpf, struct bpf_prog *prog);
...
void bpf_warn_invalid_xdp_action(struct net_device *dev, struct bpf_prog *prog,
				 u32 act);

#ifdef CONFIG_INET
struct sock *bpf_run_sk_reuseport(struct sock_reuseport *reuse, struct sock *sk,
				  struct bpf_prog *prog, struct sk_buff *skb,
				  struct sock *migrating_sk, u32 hash);
#else
...
bpf_jit_binary_pack_alloc(unsigned int proglen, u8 **ro_image,
			  unsigned int alignment,
			  struct bpf_binary_header **rw_hdr, u8 **rw_image,
			  bpf_jit_fill_hole_t bpf_fill_ill_insns);
...
static inline const char *bpf_address_lookup(unsigned long addr,
					     unsigned long *size,
...
static inline bool bpf_sk_lookup_run_v6(struct net *net, int protocol,
					...
					const __be16 sport,
					const struct in6_addr *daddr,
					const u16 dport, const int ifindex,
					struct sock **psk)
{
	struct bpf_prog_array *run_array;
	struct sock *selected_sk = NULL;
...
__bpf_xdp_redirect_map(struct bpf_map *map, u32 ifindex, u64 flags,
		       const u64 flag_mask,
