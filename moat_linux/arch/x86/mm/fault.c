		   ...
		   unsigned long address)
{
  ...
  if (hw_error_code & X86_PF_PK) {
    ...
    int key;
    pgd_t *pgd;
    printk("[MOAT] detected PKS violation\n");
    printk("[MOAT] IP is 0x%lx\n", regs->ip);
    pgd = __va(read_cr3_pa());
    key = pks_lookup_in_pgd(pgd, address, &level);
    printk("[MOAT] address is 0x%lx key is %d\n", address, key);
  }
  if(this_cpu_read(moat_fault_expected)) {
    printk("[MOAT] expected fault caught @ %lx\n", address);
    moat_vmap_range(address, address+PAGE_SIZE, virt_to_phys(moat_scratch_page), PAGE_KERNEL_EXEC, __va(read_cr3_pa()));
    return;
  }
