			...
			struct task_struct *tsk)
{
	 ...
	 * isn't free.
	 */
  /*if (this_cpu_read(cpu_tlbstate.bpf_pgd)) {
      this_cpu_write(cpu_tlbstate.bpf_pgd, 0);
      if (real_prev == next) {
        write_cr3(build_cr3_noflush(real_prev->pgd, prev_asid));
      }
  }*/
	...
	if (WARN_ON_ONCE(__read_cr3() != build_cr3(real_prev->pgd, prev_asid))) {
		 ...
		 */
    if (real_prev == next)
      printk("read_cr3(): %lx, bpf_pgd: %llx, prev: %lx\n", __read_cr3(), this_cpu_read(cpu_tlbstate.bpf_pgd), build_cr3(real_prev->pgd, prev_asid));
		__flush_tlb_all();
	}
#endif
	if (was_lazy)
	 ...
	 */
	if (real_prev == next) {
    // real_prev is what we are going next, has to switch back.
