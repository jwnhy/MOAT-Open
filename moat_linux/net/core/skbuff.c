...
struct sk_buff *__build_skb(void *data, unsigned int frag_size)
{
  ...
  skb->moat_pfn.val = pte_pfn(*lookup_address((u64)skb, &level));
  if (skb->dev != NULL)
    skb->ifindex = skb->dev->ifindex;
	return skb;
}
...
struct sk_buff *skb_clone(struct sk_buff *skb, gfp_t gfp_mask)
{
  ...
  int level;
	...
	if (skb->fclone == SKB_FCLONE_ORIG &&
		...
		if (!n)
			return NULL;
    n->moat_pfn.val = pte_pfn(*lookup_address((u64)n, &level));
    if (skb->dev != NULL)
      skb->ifindex = skb->dev->ifindex;
