			     ...
			     struct bpf_prog *xdp_prog)
{
  ...
  if (xdp->rxq)
    xdp->rx_queue_index = xdp->rxq->queue_index;
  if (xdp->rxq && xdp->rxq->dev)
    xdp->ingress_ifindex = xdp->rxq->dev->ifindex;
  if (xdp->txq && xdp->txq->dev)
    xdp->egress_ifindex = xdp->txq->dev->ifindex;
...
int do_xdp_generic(struct bpf_prog *xdp_prog, struct sk_buff *skb)
{
	if (xdp_prog) {
		...
		u32 act;
		int err;
    u64 heap = ALIGN_DOWN(*arch_raw_cpu_ptr(xdp_prog->pack.percpu_stack), PAGE_SIZE);
    xdp = (void*)heap;
