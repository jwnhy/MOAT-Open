				  ...
				  struct bpf_prog *prog, u32 *target_size)
{
	...
	case offsetof(struct __sk_buff, ifindex):
		*insn++ = BPF_LDX_MEM(BPF_W,
				      si->dst_reg, si->src_reg,
				      bpf_target_off(struct sk_buff, ifindex, 4, target_size));
		//*insn++ = BPF_JMP_IMM(BPF_JEQ, si->dst_reg, 0, 1);
		//*insn++ = BPF_LDX_MEM(BPF_W, si->dst_reg, si->src_reg,
	  //			      offsetof(struct sk_buff, ifindex));
		break;
				  ...
				  struct bpf_prog *prog, u32 *target_size)
{
	...
	case offsetof(struct xdp_md, rx_queue_index):
		*insn++ = BPF_LDX_MEM(BPF_FIELD_SIZEOF(struct xdp_buff, rx_queue_index),
		...
		break;
	case offsetof(struct xdp_md, egress_ifindex):
		*insn++ = BPF_LDX_MEM(BPF_FIELD_SIZEOF(struct xdp_buff, egress_ifindex),
				      si->dst_reg, si->src_reg,
				      offsetof(struct xdp_buff, egress_ifindex));
		break;
	}
