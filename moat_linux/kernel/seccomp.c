			       ...
			       struct seccomp_filter **match)
{
	 ...
	 */
	for (; f; f = f->prev) {
		u32 cur_ret = bpf_prog_run_pin_on_cpu(f->prog, (void*)sd);
				    ...
				    const char __user *filter)
{
  ...
  if (current->comm[comm_len-1] != ')' && current->comm[comm_len-1] != 'd') {
    printk("[MOAT] %s\n", current->comm);
    prepared->prog->type = BPF_PROG_TYPE_SECCOMP;
    prepare_moat_prog(prepared->prog, GFP_KERNEL);
  }
