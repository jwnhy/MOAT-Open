...
static int bpf_array_alloc_percpu(struct bpf_array *array)
{
  ...
  for_each_possible_cpu(i) {
    ...
    region->mem = mem;
    list_add(&region->entry, &array->map.region_list);
    *per_cpu_ptr(array->pptrs, i) = mem->addr;
...
static void array_map_free_moat_mem(struct bpf_array *array) {
  struct moat_mem *array_mem = NULL;
  struct list_head *entry, *temp;
  list_for_each_safe(entry, temp, &array->map.region_list) {
    struct moat_region* region;
    region = list_entry(entry, struct moat_region, entry);
    if (region->mem->addr != array_map_vmalloc_addr(array))
      dealloc_moat_mem(region->mem);
    else
      array_mem = region->mem;
    list_del(entry);
    kfree(region);
  }
  if (WARN_ON(!array_mem))
    dealloc_moat_mem(array_mem);
}
...
//	} else {
    ...
    nr_pages = PAGE_ALIGN(array_size) / PAGE_SIZE;
    mem = alloc_moat_mem(nr_pages, PAGE_KERNEL, GFP_KERNEL | __GFP_ZERO);
    avail_off = nr_pages * PAGE_SIZE - array_size;
    off = ALIGN(get_random_u32_below(avail_off), 8);
    array = mem->addr + off;
    if (attr->map_flags & BPF_F_MMAPABLE)
      array = mem->addr + PAGE_ALIGN(sizeof(struct bpf_array)) - offsetof(struct bpf_array, value);
//	}
	if (!array)
		return ERR_PTR(-ENOMEM);

  INIT_LIST_HEAD(&array->map.region_list);
  // for mmapable map, we didn't allocate MOAT memory
  // BUG_ON(mem && (attr->map_flags & BPF_F_MMAPABLE));
  if (mem) {
    region = kmalloc(sizeof(struct moat_region), GFP_KERNEL);
    region->mem = mem;
    list_add(&region->entry, &array->map.region_list);
...
static void *percpu_array_map_lookup_elem(struct bpf_map *map, void *key)
{
  ...
  void *ret;
	...
	if (unlikely(index >= array->map.max_entries))
		return NULL;
  ret = *this_cpu_ptr(array->pptrs) + (index & array->index_mask) * array->elem_size;
  //printk("[MOAT] lookup percpu map @ %d is %llx\n", smp_processor_id(), (u64)ret);
	return ret;
}
			    ...
			    u64 map_flags)
{
	...
	for_each_possible_cpu(cpu) {
		...
		check_and_free_fields(array, *per_cpu_ptr(pptr, cpu) + (index & array->index_mask) * array->elem_size);
		off += size;
	}
	rcu_read_unlock();
	return 0;
	...
	if (map_value_has_kptrs(map)) {
		if (array->map.map_type == BPF_MAP_TYPE_PERCPU_ARRAY) {
			for (i = 0; i < array->map.max_entries; i++) {
				...
				for_each_possible_cpu(cpu) {
					bpf_map_free_kptrs(map, *per_cpu_ptr(pptr, cpu) + (i & array->index_mask) * array->elem_size);
					cond_resched();
				}
			}
		} else {
...
{
	struct bpf_array *array = container_of(map, struct bpf_array, map);
  void* array_page = array_map_vmalloc_addr(array);
  ...
  if (!array_mem)
    return -EINVAL;
  return moat_mem_mmap(vma, array_mem, vma->vm_pgoff + pgoff);
