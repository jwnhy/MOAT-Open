			     ...
			     int *insn_idx_p)
{
  ...
  struct bpf_insn_aux_data *aux = NULL;
  int arg_idx;
  ...
  aux = cur_aux(env);
    ...
    arg_idx = BPF_REG_1+i;
		...
		if (err)
			return err;
    switch (fn->arg_type[i]) {
      /* scalar case */
      case ARG_CONST_SIZE:
      case ARG_CONST_ALLOC_SIZE_OR_ZERO:
      case ARG_CONST_SIZE_OR_ZERO:
        aux->arg_ranges = vzalloc(sizeof(struct bpf_arg_range) * 5);
        aux->arg_ranges[i].umin = regs[arg_idx].umin_value;
        aux->arg_ranges[i].umax = regs[arg_idx].umax_value;
        aux->arg_ranges[i].valid = true;
        pr_err("[MOAT]: inst @ %d arg %d %s aux [%llu, %llu]", env->insn_idx, i, func_id_name(func_id), aux->arg_ranges[i].umin, aux->arg_ranges[i].umax);
        break;
      /* TODO: other cases */
      default:
        break;
...
err_unlock:
	if (!is_priv)
		mutex_unlock(&bpf_verifier_lock);
  env->prog->insn_aux = env->insn_aux_data;
  /*
  for (i = 0; i < env->prog->len; i++) {
    if (env->insn_aux_data->arg_ranges)
      vfree(env->insn_aux_data->arg_ranges);
  }
