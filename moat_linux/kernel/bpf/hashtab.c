...
#define BATCH_OPS(_name)                                                    \
	.map_lookup_batch = _name##_map_lookup_batch,                       \
	.map_lookup_and_delete_batch = _name##_map_lookup_and_delete_batch, \
...
static inline void htab_elem_set_ptr(struct htab_elem *l, u32 key_size,
				     void *pptr, int cpu)
{
	*(void **)(l->key + key_size + cpu * sizeof(void *)) = pptr;
}

static inline void *htab_pptr_get_ptr(void** pptr, int cpu) {
  return *(void**)((void*)pptr + sizeof(void*) * cpu);
}

static inline void **htab_elem_get_ptr(struct htab_elem *l, u32 key_size,
				       int cpu)
{
	return (void **)(l->key + key_size + cpu * sizeof(void *));
}
...
free_elems: */
	list_for_each_safe(entry, temp, &htab->map.region_list) {
		struct moat_region *region;
		region = list_entry(entry, struct moat_region, entry);
		dealloc_moat_mem(region->mem);
		list_del(entry);
		kfree(region);
	}
...
static int prealloc_init(struct bpf_htab *htab)
{
	...
	// regardless of percpu or not, this should be initialize
	INIT_LIST_HEAD(&htab->map.region_list);
	nr_pages = PAGE_ALIGN((u64)htab->elem_size * num_entries) / PAGE_SIZE;
	mem = alloc_moat_mem(nr_pages, PAGE_KERNEL, GFP_KERNEL | __GFP_ZERO);
	htab->elems = mem->addr;
	region = kmalloc(sizeof(struct moat_region), GFP_KERNEL);
	region->mem = mem;
	list_add(&region->entry, &htab->map.region_list);
	if (!mem)
		return -ENOMEM;

	//htab->elems = bpf_map_area_alloc((u64)htab->elem_size * num_entries, htab->map.numa_node);

	if (!htab_is_percpu(htab))
		goto skip_percpu_elems;

	htab->percpu_elems = bpf_map_alloc_percpu(&htab->map, sizeof(void **),
						  8, GFP_USER | __GFP_NOWARN);
	if (!htab->percpu_elems)
		return -ENOMEM;
	value_size = round_up(htab->map.value_size, 8);
	nr_pages = PAGE_ALIGN((u64)value_size * num_entries) / PAGE_SIZE;

	for_each_possible_cpu(i) {
		mem = alloc_moat_mem(nr_pages, PAGE_KERNEL,
				     GFP_KERNEL | __GFP_ZERO);
		region = kmalloc(sizeof(struct moat_region), GFP_KERNEL);
		region->mem = mem;
		list_add(&region->entry, &htab->map.region_list);
		*per_cpu_ptr(htab->percpu_elems, i) = mem->addr;
		if (!mem)
			return -ENOMEM;
	}
	for (i = 0; i < num_entries; i++) {
    ...
    void **temp;
		for_each_possible_cpu(j) {
			void *ptr = *per_cpu_ptr(htab->percpu_elems, j) +
				    i * value_size;
			htab_elem_set_ptr(elem, htab->map.key_size, ptr, j);
      temp = htab_elem_get_ptr(elem, htab->map.key_size, j);
      //printk("[MOAT] setting %llx, getting %llx\n", (u64)ptr, (u64)*temp);
		}
		cond_resched();
	}
...
static struct bpf_map *htab_map_alloc(union bpf_attr *attr)
{
	...
	if (percpu_lru) {
		...
		htab->map.max_entries =
			roundup(attr->max_entries, num_possible_cpus());
	...
	htab->elem_size =
		sizeof(struct htab_elem) + round_up(htab->map.key_size, 8);
	// have to replicate per-cpu in this way... don't like this.
	if (percpu)
		htab->elem_size += sizeof(void *) * num_possible_cpus();
	else
	...
	htab->buckets = bpf_map_area_alloc(
		htab->n_buckets * sizeof(struct bucket), htab->map.numa_node);
	if (!htab->buckets)
		goto free_htab;

	for (i = 0; i < HASHTAB_MAP_LOCK_COUNT; i++) {
		htab->map_locked[i] = bpf_map_alloc_percpu(
			&htab->map, sizeof(int), sizeof(int), GFP_USER);
...
__htab_lru_map_lookup_elem(struct bpf_map *map, void *key, const bool mark)
{
	struct htab_elem *l = __htab_map_lookup_elem(map, key);
...
{
	if (htab->use_percpu_counter)
		percpu_counter_add_batch(&htab->pcount, 1,
...
static void pcpu_copy_value(struct bpf_htab *htab, void **pptr, void *value,
			    bool onallcpus)
{
	int current_cpu = raw_smp_processor_id();
	if (!onallcpus) {
		/* copy true value_size bytes */
		void *ptr = htab_pptr_get_ptr(pptr, current_cpu);
		memcpy(ptr, value, htab->map.value_size);
	} else {
		u32 size = round_up(htab->map.value_size, 8);
		int off = 0, cpu;

		for_each_possible_cpu(cpu) {
			void *ptr = htab_pptr_get_ptr(pptr, cpu);
			bpf_long_memcpy(ptr, value + off, size);
			off += size;
		}
	}
}

static void pcpu_init_value(struct bpf_htab *htab, void **pptr, void *value,
			    bool onallcpus)
{
	...
	if (!onallcpus) {
		...
		for_each_possible_cpu(cpu) {
			void *ptr = htab_pptr_get_ptr(pptr, cpu);
			if (cpu == current_cpu)
				bpf_long_memcpy(ptr, value, value_size);
			else
					 ...
					 struct htab_elem *old_elem)
{
	...
	if (percpu) {
		if (prealloc) {
			...
			pptr = htab_elem_get_ptr(l_new, key_size, 0);
		} else {
			BUG();
		}
		/* prealloc is ALWAYS true.
    else {
		...
		copy_map_value(&htab->map, l_new->key + round_up(key_size, 8),
			       value);
	}
	...
	if (unlikely(map_flags & BPF_F_LOCK)) {
		...
		if (l_old) {
			...
			copy_map_value_locked(
				map, l_old->key + round_up(key_size, 8), value,
				false);
			return 0;
		}
				    ...
				    u64 map_flags)
{
	...
	if (!l_new)
		return -ENOMEM;
	copy_map_value(&htab->map, l_new->key + round_up(map->key_size, 8),
		       value);
					 ...
					 bool onallcpus)
{
	...
	if (l_old) {
		...
		pcpu_copy_value(htab, htab_elem_get_ptr(l_old, key_size, 0),
				value, onallcpus);
	} else {
		l_new = alloc_htab_elem(htab, key, value, key_size, hash, true,
					onallcpus, NULL);
		if (IS_ERR(l_new)) {
			ret = PTR_ERR(l_new);
			goto err;
					     ...
					     bool onallcpus)
{
	...
	if (l_old) {
		...
		pcpu_init_value(htab, htab_elem_get_ptr(l_new, key_size, 0),
				value, onallcpus);
		hlist_nulls_add_head_rcu(&l_new->hash_node, head);
		l_new = NULL;
	}
					     ...
					     bool is_percpu, u64 flags)
{
	...
	if (!l) {
		...
		if (is_percpu) {
			...
			pptr = htab_elem_get_ptr(l, key_size, 0);
			for_each_possible_cpu(cpu) {
        void *ptr = htab_pptr_get_ptr(pptr, cpu);
				bpf_long_memcpy(value + off, ptr,
						roundup_value_size);
			...
			if (flags & BPF_F_LOCK)
				copy_map_value_locked(map, value,
						      l->key + roundup_key_size,
						      true);
			else
				copy_map_value(map, value,
					       l->key + roundup_key_size);
...
again_nocopy:
	...
	hlist_nulls_for_each_entry_safe(l, n, head, hash_node) {
		...
		if (is_percpu) {
			...
			pptr = htab_elem_get_ptr(l, map->key_size, 0);
			for_each_possible_cpu(cpu) {
				void *ptr = htab_pptr_get_ptr(pptr, cpu); 
				bpf_long_memcpy(dst_val + off, ptr, size);
				off += size;
			}
		} else {
			...
			if (map->map_type == BPF_MAP_TYPE_HASH_OF_MAPS) {
				...
				map_id = map->ops->map_fd_sys_lookup_elem(
					*inner_map);
				value = &map_id;
			}
...
static int htab_percpu_map_lookup_and_delete_batch(struct bpf_map *map,
						   ...
						   union bpf_attr __user *uattr)
{
	return __htab_map_lookup_and_delete_batch(map, attr, uattr, true, false,
						  true);
}
...
static int htab_lru_percpu_map_lookup_batch(struct bpf_map *map,
					    ...
					    union bpf_attr __user *uattr)
{
	return __htab_map_lookup_and_delete_batch(map, attr, uattr, false, true,
						  true);
}
...
htab_lru_percpu_map_lookup_and_delete_batch(struct bpf_map *map,
					    ...
					    union bpf_attr __user *uattr)
{
	return __htab_map_lookup_and_delete_batch(map, attr, uattr, true, true,
						  true);
}
	...
	if (prog) {
		...
		if (elem) {
			...
			if (!info->percpu_value_buf) {
				...
				for_each_possible_cpu(cpu) {
					void *ptr =
            htab_pptr_get_ptr(pptr, cpu);
					bpf_long_memcpy(
						info->percpu_value_buf + off,
						ptr, roundup_value_size);
					off += roundup_value_size;
				}
...
static int bpf_for_each_hash_elem(struct bpf_map *map,
...
static void *htab_percpu_map_lookup_elem(struct bpf_map *map, void *key)
{
	...
	if (l)
		return *htab_elem_get_ptr(l, map->key_size, current_cpu);
	else
		return NULL;
}
						...
						u32 cpu)
{
	...
	if (l)
		return *htab_elem_get_ptr(l, map->key_size, cpu);
	else
		return NULL;
}
...
int bpf_percpu_hash_copy(struct bpf_map *map, void *key, void *value)
{
	...
	for_each_possible_cpu(cpu) {
    void *ptr = htab_pptr_get_ptr(pptr, cpu);
		bpf_long_memcpy(value + off, ptr, size);
		off += size;
	}
	ret = 0;
out:
