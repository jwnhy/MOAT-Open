...
#include <unistd.h>
#include "sockex4.skel.h"

static struct env {
	const char *interface;
} env;

struct sockex4_bpf *skel;
...
static void sig_handler(int sig)
{
	...
	struct pair value;
	key = IPPROTO_TCP;
	while (bpf_map__get_next_key(skel->maps.hash_map, &key, &next_key, sizeof(key)) == 0) {
		bpf_map__lookup_elem(skel->maps.hash_map, &next_key, sizeof(next_key), &value,
				     sizeof(value), 0);
...
int main(int argc, char **argv)
{
	...
	skel = sockex4_bpf__open_and_load();
	if (!skel) {
	...
	prog_fd = bpf_program__fd(skel->progs.sockex4);
